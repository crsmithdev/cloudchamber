...
def export(root: str | Path = ".", out: str | Path | None = None,
    ...
    """Write the kept set to `extracted/examples.md` in the documented format."""
    out_dir = Path(out or Path(root) / "extracted")
    bank = Bank(out_dir)
    kept = sorted(bank.kept(), key=lambda p: (cell(p.get("facets")),
                                              -p.get("score", 0)))
    dest = Path(path) if path else out_dir / "examples.md"

    lines = [
        "# EXAMPLES — register conditioning",
        "",
        "*Generated by `pipeline export`. Chris's keeps from "
        "`examples.jsonl`; do not hand-edit — edit the decisions instead.*",
        "",
        f"{len(kept)} passages.",
        "",
    ]
