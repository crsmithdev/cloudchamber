...
Then draw at least one of the four §1.1 pulls the same way — a theme, a dread mechanism, an artifact, or a setting element picked at random rather than chosen. The setting element is a number into lore §3 (for the setting-a, that resolves to playbook §5); the artifact comes from lore §4 or playbook §4.2.
