...
3. **Name the engine, and turn something that works.** Two halves of one move, and this is where the darkness actually comes from — not from step 4.
   **The engine** is the process the institution is administering: something operating on bodies or minds at population scale, describable mechanically, which would go on existing if the institution vanished. If all you have is an organisation applying a rule to people, there is no engine, and what you have is a setting. Go back to step 1.
...
5. **Say what it does to a body, and why nobody stops it.** In the specific — a stage, a posture, a duration, a count, a temperature. Then the reasons nobody prevents it, which have to be mundane rather than sinister: the presentation is atypical, the window closed before anyone knew they were in it, the standard predates the thing, the funding is annual, the criteria select for exactly the wrong people. If you cannot say what it physically does, and why it cannot be treated, refused or walked out of, you have an institution and not a horror. **Write the worst version first and decide afterwards what to pull back from — never the reverse.**
