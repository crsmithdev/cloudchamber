...
export function Queue() {
  ...
  const load = useCallback(async () => {
    const q = await api.queue(kind, source || undefined);
    setItem(q.items[0] ?? null); setRemaining(q.remaining); setArtifact(false); setNote("");
  }, [kind, source]);
  ...
  return (
    <>
      <div className="row meta">
        ...
        <span>{remaining} unreviewed</span>
      ...
      {!item ? <div className="card meta">Nothing unreviewed{source ? ` in ${source}` : ""}.</div> : (
        <div className="card">
          <div className="meta">{kind === "example" ? <>{item.source} · {item.title} — {item.author || "unknown"} · {item.genre} · <code>{item.cell}</code> · {item.words}w · {item.id}</> : <>×{item.attestation} · {item.stories && JSON.parse(item.stories).join(", ")} · {item.id}</>}</div>
