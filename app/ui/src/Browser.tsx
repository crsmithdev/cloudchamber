  ...
  return (
    <>
      <div className="row meta">
        <select value={kind} onChange={(e) => { setKind(e.target.value); setF({}); setOffset(0); }}><option value="example">examples</option><option value="theme">themes</option><option value="packet">packets</option></select>
        {kind === "example" && facets && <>
        ...
        <select value={f.artifact ?? ""} onChange={set("artifact")}><option value="">artifact?</option><option value="true">flagged</option><option value="false">not flagged</option></select>
        <span style={{ marginLeft: "auto" }}>{res.total} items</span>
      </div>
      <table>
        <thead><tr><th style={{ width: "40%" }}>text</th><th>where</th><th>cell</th><th>verdict</th><th></th></tr></thead>
        <tbody>{res.items.map((it) => (
          <tr key={it.id} onClick={() => setOpen(open === it.id ? null : it.id)} style={{ cursor: "pointer" }}>
            ...
            <td className="meta">{kind === "example" ? <>{it.source}<br />{it.title} — {it.author}</> : kind === "theme" ? <>×{it.attestation}<br />{it.stories && JSON.parse(it.stories).join(", ")}</> : <>{it.setting ?? "unrestricted"} · {it.genre}<br /><a href={`#run/${it.id}`}>{it.id}</a></>}</td>
            <td><code>{it.cell ?? ""}</code></td>
            ...
            <td className="row" onClick={(e) => e.stopPropagation()}><button className="keep" onClick={() => decide(it, "keep")}>k</button><button className="pass" onClick={() => decide(it, "pass")}>p</button><button className="flag" onClick={() => decide(it, it.latest?.verdict ?? "keep", !it.latest?.artifact)}>a</button></td>
          </tr>))}</tbody>
      </table>
