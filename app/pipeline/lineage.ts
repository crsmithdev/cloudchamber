 ...
 * `forked_from`, and a draw another replaced names it in `superseded_by`. Every
 * question those links answer is answered here, from the draw rows: the chain
 * back to its root, the rounds, which draws are tips, what repairs or forks a
 * draw, what points at it, and how it was superseded.
 *
...
export class Lineage {
  ...
  /** Every draw pointing at this one: what repaired it, forked it, or superseded it. */
  referencedBy(id: string): string[] {
    const out = new Set([...this.repairs(id), ...this.forks(id)]);
  ...
  /** This draw and every draw it came from, through repairs and forks, back to the one that ran the premises. A link that loops ends the walk. */
  path(id: string): string[] {
    const ids: string[] = [];
    for (let cur: string | null = id; cur && !ids.includes(cur); ) {
      ids.push(cur);
      const r = this.row(cur);
      cur = r.repaired_from ?? r.forked_from;
    }
    return ids;
  }
