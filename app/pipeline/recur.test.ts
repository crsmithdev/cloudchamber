...
describe("recurrence", () => {
  ...
  test("a cluster is reported at keep_if distinct samples; spans at 0.5 or statements at 0.6 join", () => {
    const fs = [
      f("ledger", 1, "the director fires the reliquary", "the director fires it", { invalidates: "departure" }),
      ...
      f("ledger", 1, "twelfth relic named the Verona clavicle", "the relic is named twice", { invalidates: "particulars" }),
      f("ledger", 3, "the twelfth relic named the Verona clavicle", "the relic is named twice"),
      f("ledger", 2, "tears on the silk", "the tears were cut", { invalidates: "knowledge" }),
    ...
    expect(cs.map((c) => [c.n, c.invalidates, c.reported])).toEqual([[3, "departure", true], [3, "particulars", true], [1, "knowledge", false]]);
  ...
  test("ordering is n descending then departure, particulars, knowledge, an unknown section, none", () => {
    const fs = [f("d", 1, "a a a", "s1", { invalidates: "none" }), f("d", 1, "b b b", "s2", { invalidates: "knowledge" }), f("d", 1, "c c c", "s3", { invalidates: "matrix" }), f("d", 1, "d d d", "s4", { invalidates: "departure" }), f("d", 1, "e e e", "s5", { invalidates: "particulars" }), f("d", 2, "a a a", "s1", { invalidates: "none" })];
    expect(cluster(fs, 1).map((c) => c.invalidates)).toEqual(["none", "departure", "particulars", "knowledge", "matrix"]);
  });
  ...
  test("score weighs recurrence, cross-checker agreement, severity, result kind and evidence", () => {
    const s = (over: Partial<Parameters<typeof score>[0]>, samples = 3) =>
      score({ n: 3, checkers: ["ledger"], invalidates: "none", result: "contradicted", evidence: "a quote", ...over }, samples);
    expect(s({ invalidates: "departure", checkers: ["ledger", "derivation"] })).toBe(9);    // 3 + 2 + 2 + 2
    expect(s({ invalidates: "particulars" })).toBe(6);                                       // 3 + 0 + 1 + 2: a sum alone does not reach the floor
    expect(s({ invalidates: "particulars", n: 2 })).toBe(5);                                 // one sample short
    expect(s({ invalidates: "particulars", n: 1 })).toBe(4);
    expect(s({ invalidates: "knowledge", result: "underived" })).toBe(5);                     // 3 + 0 + 1 + 1
    ...
    expect(s({ invalidates: "matrix" })).toBe(5);                                           // an unknown section is worth nothing
    expect(s({ invalidates: "departure", checkers: ["ledger", "derivation"], result: "contradicts:the ledger says otherwise" })).toBe(9);
    expect(s({ n: 1 }, 1)).toBe(5);                                                          // one sample is full recurrence
    expect(s({ result: "supported", evidence: "none", n: 1 })).toBe(0);                      // never below zero
  });

  test("a hedge before a number makes an particulars finding worth no severity", () => {
    const s = (span: string, invalidates = "particulars") =>
    ...
    expect(s("roughly 1,200 steps a day", "departure")).toBe(7);          // the guard is particulars only
    // the outline's own sums are not estimates: a span quoted from it keeps its severity
    const outline = "## particulars\n\nFive nights × 3.8 is about 19 days, which is why residents take two to three weeks.";
    const ctx = { outline, prose: "about 19 days in the notebook, she wrote", ledger: "" };
    const inOutline = (span: string) => score({ n: 3, checkers: ["derivation"], invalidates: "particulars", result: "contradicted", evidence: "5 × 3.8 = 19, but a night is 8 h", span }, 3, ctx);
  ...
  test("parseFindings reads the tag shape and drops findings without a span", () => {
    const t = `<finding><span>a quote</span><statement>s</statement><result>contradicted</result><evidence>none</evidence><invalidates>Departure</invalidates><replacement>r</replacement></finding><finding><statement>no span</statement></finding>`;
    const fs = parseFindings(t, "ledger", 2);
    expect(fs).toHaveLength(1);
    expect(fs[0]).toMatchObject({ checker: "ledger", sample: 2, span: "a quote", invalidates: "departure", evidence: "none" });
  });
});
