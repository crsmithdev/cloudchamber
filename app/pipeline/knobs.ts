 ...
 * tomls, the sources from the store. `fogbelt help` prints this after the
...
export function knobs(db: Db, settingsDir: string = SETTINGS): Section[] {
  ...
  return [
    {
      title: "draw",
      note: "fogbelt draw, POST /api/draws, and the Voice Bridge draw tool take the same knobs.",
      rows: [
    ...
    {
      title: "drafting",
      note: "fogbelt draft; every key of draft.toml is overridable per draw, and a profile bundles overrides.",
      rows: [
...
export function asMarkdown(sections: Section[]): string {
  return ["# Fog Belt knobs", "", "Generated by `fogbelt help --md`; every value here is read from the setting files, the tomls and the store when that runs.", "",
